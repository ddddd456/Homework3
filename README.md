# Homework3
Ниже — готовое решение домашнего задания. Я исходил из допущения, что **один предмет ведёт один преподаватель** (`subject_id → teacher_id`). Если по условию преподавателей у предмета может быть несколько, схему нужно будет расширить таблицей связи `subject_teachers`.
